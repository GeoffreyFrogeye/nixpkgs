...
let
  ...
  postgresEnv = if isUnixSocket then {
...
in {
  options.services.immich = {
    ...
    mediaLocation = mkOption {
      ...
      description = "Directory used to store media files. If this directory already exists the immich user has to be able to read and write to it.";
    };
    ...
    machine-learning = {
      enable = mkEnableOption "immich-machine-learning" // { default = true; };
      environment = mkOption {
        type = types.submodule {
          freeformType = types.attrsOf types.str;
        };
    ...
    database = {
      enable = mkEnableOption "postgresql" // { default = true; };
      createDB = mkOption {
        type = types.bool;
        default = true;
    ...
    redis = {
      enable = mkEnableOption "immich redis server" // { default = true; };
      host = mkOption {
        type = types.str;
        default = "localhost";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.immich-server = {
      ...
      environment = lib.mkMerge [ postgresEnv {
